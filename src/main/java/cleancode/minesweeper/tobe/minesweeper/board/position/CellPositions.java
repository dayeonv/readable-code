...
public class CellPositions {
    ...
    public List<CellPosition> extractRandomPositions(int count) {
        List<CellPosition> cellPositions = new ArrayList<>(positions); // 새로운 리스트를 만들어서

        Collections.shuffle(cellPositions); // 셔플을 하고
        return cellPositions.subList(0, count); // 카운터 갯수만큼 자르기
    }
